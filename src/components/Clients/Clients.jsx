import React, { useRef } from 'react'
import Slider from 'react-slick'
import styled from 'styled-components'
import ClientSlider from './ClientSlider';
import { IoIosArrowBack, IoIosArrowForward } from "react-icons/io";
import { Slide } from 'react-awesome-reveal';

let clients = [
    {
        name : "React.js",
        position : "Avançado",
        img_url : "https://d2gbo5uoddvg5.cloudfront.net/images/modules/react.gif",
        stars : 5,
        disc : `Larga capacidade de construir qualquer aplicação web com 3 anos de prática`
    },
    {
        name : "Prisma",
        position : "Intermediario",
        img_url : "https://avatars.githubusercontent.com/u/17219288?s=280&v=4",
        stars : 4,
        disc : `Principal ORM utilizada para construções das minhas APIs`
    },
    {
        name:"Typescript",
        position : "Avançado",
        img_url:"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRkgGpTMoYPuEDv1GP_iWafWcQpvA4mGm__qA&usqp=CAU",
        stars : 4,
        disc : `Largamente utilizada em praticamente todas as aplicações que construo.`
    },
    {
        name : "Next.js",
        position : "Intermediario",
        img_url : "https://www.rlogical.com/wp-content/uploads/2023/03/Rlogical-Blog-Images-thumbnail.webp",
        stars : 2,
        disc : `Bons conhecimentos e prática, construções de algumas aplicações.`
    },
    {
        name : "Git",
        position : "Intermediario",
        img_url : "https://pachecoandre.com.br/assets/imgs/posts/git.png",
        stars : 3,
        disc : `Conhecimento e prática das principais funções utilizadas nas minhas aplicações`
    },
    {
        name:"Redux",
        position : "Avançado",
        img_url:"data:image/png;base64,",
        stars : 5,
        disc : `Largamente utilizado em praticamente todas as aplicações que construo.`
    },
]
var settings = {
    dots: true,
    infinite: true,
    speed: 500,
    slidesToShow: 3,
    slidesToScroll: 1,
    initialSlide: 0,
    arrows : false,
    responsive: [
      {
        breakpoint: 990,
        settings: {
          slidesToShow: 2,
          slidesToScroll: 1,
          infinite: true,
          dots: true
        }
      },
      {
        breakpoint: 600,
        settings: {
          slidesToShow: 2,
          slidesToScroll: 1,
          initialSlide: 2
        }
      },
      {
        breakpoint: 530,
        settings: {
          slidesToShow: 1,
          slidesToScroll: 1
        }
      }
    ]}

const Clients = () => {
    const arrowRef = useRef(null);
    let clientDisc = "";
    clientDisc = clients.map((item, i) => (
        <ClientSlider item={item} key={i}/>
    ))
  return (
    <Container id='client'>
        <Slide direction="left">
            <span className="green">Habilidades</span>
            <h1>Principais stacks</h1>
        </Slide>
        <Testimonials>
            <Slider ref={arrowRef} {...settings}>
                {clientDisc}
            </Slider>
            <Buttons>
                <button
                onClick={() => arrowRef.current.slickPrev()}
                ><IoIosArrowBack/></button>
                <button
                onClick={() => arrowRef.current.slickNext()}
                ><IoIosArrowForward/></button>
            </Buttons>
        </Testimonials>
    </Container>
  )
}

export default Clients

const Container = styled.div`
    width: 80%;
    max-width: 1280px;
    margin: 0 auto;
    padding: 4rem 0;

    @media(max-width:840px){
        width: 90%;
    }

    span{
        font-weight: 700;
        text-transform: uppercase;
    }

    h1{
        padding-top: 1rem;
        text-transform: capitalize;
    }

    .slick-list, .slick-slider, .slick-track{
        padding: 0;
    }

    .slick-dots{
        text-align: left;
        margin-left: 1rem;
    }

    .slick-dots li button:before{
        content: "";
    }

    .slick-dots li button{
        width: 9px;
        height: 4px;
        background: linear-gradient(159deg, rgb(45, 45, 58) 0%, rgb(43, 43, 53) 100%);
        padding: 0.1rem;
        margin-top: 1rem;
        transition: all 400ms ease-in-out;
        border-radius: 50px;
    }
    
    .slick-dots li.slick-active button{
        background: #01be96;
        width: 15px;
    }

    .slick-dots li{
        margin: 0;
    }
`

const Testimonials = styled.div`
    margin-top: 2rem;
    position: relative;
`
const Buttons = styled.div`
    position: absolute;
    right: 0.7rem;
    bottom: -2rem;

    button{
        background-color: transparent;
        margin-left: 0.5rem;
        border: none;
        color: #01be96;
        cursor: pointer;
        font-size: 1.1rem;
    }

    @media(max-width:530px){
        display: none;
    }
`